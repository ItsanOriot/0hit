...
VisualsSecond:AddDivider("Third Person");
VisualsSecond:AddBind({text = "On / Off", skipflag = true, nomouse = true, key = "Pageup", callback = function(v)
	thirdperson = v
	game.Players.LocalPlayer.CameraMaxZoomDistance = 0
	game.Players.LocalPlayer.CameraMinZoomDistance = 0
end});
VisualsSecond:AddSlider{text = "Distance", skipflag = true, min = 0, max = 30, value = 1, callback = function(v)
	...
	if killAll then
		for _, plr in next, game:GetService("Players"):GetChildren() do
			if plr.Team ~= lp.Team then
				local args = {
					[1] = plr.Character.Head,
					[2] = Vector3.new(plr.Character.Head.Position),
					[3] = "AWP",
					[4] = 8192,
					...
					[9] = false,
					[10] = false,
					[11] = Vector3.new(-297.1992492675781, 51.10768508911133, 4.124560356140137),
					[12] = 807,
					[13] = Vector3.new(0.7280203104019165, 0.6834508776664734, 0.05368006229400635),
					[14] = true,
					[15] = 36,
					[16] = 86
				}
				game:GetService("ReplicatedStorage").Events.EEED:FireServer(unpack(args))
			end
		end
	end
end)
...
game:GetService("RunService").RenderStepped:Connect(function()
	if rageMain then
		for _, plr in next, game:GetService("Players"):GetChildren() do
			local pos, vis = workspace.CurrentCamera:WorldToScreenPoint(Vector3.new(plr.Character.Head.Position))
...
game:GetService("RunService").RenderStepped:Connect(function()
	if bhop == true then
        if UserInputService:IsKeyDown(Enum.KeyCode.Space) then
			...
			game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame + game.Players.LocalPlayer.Character.Humanoid.MoveDirection * (bhopSpeed * 0.007)
...
game:GetService("RunService").RenderStepped:Connect(function()
	if voiceSpam then
		if math.random(1, 10) == 3 then
			game:GetService("ReplicatedStorage").Events.SendVoice:FireServer(unpack(voiceArgs))
		end
	end
end)
